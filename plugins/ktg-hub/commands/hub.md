...
2. **Run the bundled pipeline.** Execute exactly (note the leading env var so the bundled house voice resolves regardless of CWD):
   ...
   - Use `${CLAUDE_PLUGIN_ROOT}` for the pipeline path, the bundled config, and the bundled voice file. Never hardcode an absolute path.
   - `KTG_VOICE_FILE` and `--config` are what make the bundled pipeline portable when the plugin is installed on a machine whose CWD is some other project.

3. **Summarize the publish kit.** Read the generated output under `pipeline/output/<slug>/` (the 8-file publish kit: per-platform variants — Medium / Reddit / X / LinkedIn / Meta — plus SEO + ads + schema artifacts) and give the user a concise summary of each piece. Note the slug.

4. **STOP at the review gate.** This is non-bypassable.
   - Do **NOT** auto-publish anything.
   - Write an approval-needed state for the slug so the gate survives `/clear` (e.g. a note in `pipeline/output/<slug>/` indicating the kit is assembled and awaiting per-post `YES`). Do **NOT** create the `.approved` marker yourself — that marker is written only by `publish-reviewer` after an explicit human `YES`.
